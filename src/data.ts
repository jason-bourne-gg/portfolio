...
export const projects: Project[] = [
  {
    ...
    title: "agent-evals",
    desc: "An eval and observability harness for LLM agents. Records model calls once and replays them, so the suite runs from fixtures with no API key — and its own CI runs it on every push. Judges pairwise in both presentation orders, reports the flip rate as the judge's error bar, and scores the trajectory, not just the final answer.",
    ...
    note: "Live, the same suite drops to 28% and the larger model scores worse than the smaller one — the extra failures are brittle string assertions, not a misbehaving agent. Left as they are, and documented.",
  },
  {
    rank: "A+",
    kicker: "// FULL-STACK · REAL CUSTOMER",
    title: "Plotting Society",
    desc: "Plot owners rarely live near the land they bought, so every question becomes a phone call to the builder's sales desk. This replaces it. Built against a real 823-plot project in Nagpur: a live layout map, an append-only ledger, and owner queries on a visible SLA clock.",
  ...
  },
  {
    ...
    title: "Genesis",
    desc: "Describe an internal tool in a chat box and Claude writes it, streaming into a Monaco editor while a sandboxed preview runs it against your own HighLevel contacts and calendars. Generated code is untrusted throughout: opaque origin, no tokens in the browser, every call re-checked server-side.",
